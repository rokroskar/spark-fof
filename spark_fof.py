...
class FOFAnalyzer(): 
    ...
    def get_gid_map(self, level=0): 
        ...
        pg_map = (particle_rdd.mapPartitionsWithIndex(lambda index, data: get_buffer_particles(index,data,domain_containers,level))
                           .map(pid_gid)
                           .collectAsMap())
        ...
        # 3. from each group list, generate a (g, g') key, value pair RDD where g maps onto g'
    ...
    def get_level_map(self, level=0):
        ...
        # get the initial group mapping across sub-domains just based on particle IDs 
        ...
        if len(groups_map) > 0: 
            ...
            for sg in nx.connected.connected_component_subgraphs(g): 
                if len(sg) > 1: 
                    ...
                    new_mapping = {node: base_node for node in sg.nodes() if node != base_node}
